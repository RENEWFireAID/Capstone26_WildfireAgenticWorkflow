...
def search_collection(
    query_embedding: list[float], collection: str, top_k: int
) -> list[Dict[str, Any]]:
    """
    ...
    try:
        ...
        raise RetrievalError(
            f"Error querying ChromaDB collection '{collection}': {e}"
        ) from e
    ...
    for i, chunk_id in enumerate(ids):
        ...
        raw_results.append({"chunk_id": chunk_id, "text": text, "metadata": meta})
    ...
    for i, res in enumerate(raw_results):
        ...
        if not doc_id:
            doc_id = (
                chunk_id.split("::")[0]
                if "::" in chunk_id
                else res.get("doc_id", "unknown_doc")
            )
